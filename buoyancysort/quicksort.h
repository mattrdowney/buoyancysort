	...
	void not_quicksort(Type *data, long before_first, long after_last, long cutoff, bool even)
	{
		if (after_last - (before_first + 1) > cutoff)
		{
			...
			if (even)
			{
				...
				// I think I figured out how to implement velocitysort. If an element moves say 100 elements, that information is only useful on the next iteration if it moves in the same direction again. When this happens you know it's moving really fast that way for a reason, so you bin it far along in that direction. I'm not sure what to do when elements go in the opposite direction, but there's definitely statistical ways to do cool things (I'm just not a statistician).
				// I feel like a lot of this has to do with conditional probabilities. What's the probability that if an element moved 10 then 100 then 1000 positions it will move 10,000 positions next iteration.
				// You bin based on these probabilities and call it a day. Quicksort is my favorite skeleton for Velocitysort because it allows for a "global" comparison to sample against.
				// After a few iterations to collect information and do statistical comparisons, you do a adaptive mergesort and call it a day.
